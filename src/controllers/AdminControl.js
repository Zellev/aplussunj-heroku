...
const { paginator, shuffleArray, todaysdate, dateFull, 
        hashed, getFileStream, deleteFiles } = require('../helpers/global');
const { userValidator, dosenValidator, mahasiswaValidator, 
...
module.exports = {
  ...
  async putUserbulk(req, res, next) {
    try {console.log('here')
      ...
      let updateVal = [];
      const excelFile = pathAll(req.file.key, 'xlsx');
      ...
      if(req.file) {
        deleteFiles([{Key:`excels/${req.file.key}`}]);
      }
      next(error);
    }
  },
  ...
  async searchMhs(req, res, next) {
    try {
      let { find } = req.query;
      const validator = mahasiswaValidator(find);
      if (validator instanceof createError) throw validator;
      let mhs = [];
      const pages = parseInt(req.query.page);
      const limits = parseInt(req.query.limit);
