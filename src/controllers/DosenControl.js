...
const { createKode, paginator, paginatorMN, shuffleArray, 
        dateFull, todaysdate, getFileStream, uploadFile, deleteFiles } = require('../helpers/global');
...
const pathAll = (filename, filetype) => {
  if(filetype === 'xlsx'){
    return getFileStream(filename, 'xlsx');
...
module.exports = {
  ...
  async setSoal(req, res, next){
    try {
      ...
      if(matkulExist){
        ...
        if(gambar_soal_1){vals.push(gambar_soal_1[0].key)}
        if(gambar_soal_2){vals.push(gambar_soal_2[0].key)}
        if(gambar_soal_3){vals.push(gambar_soal_3[0].key)}
        if(req.body.audio_soal) {
          audio_soal = req.body.audio_soal;
        } else if(audio_soal) {
          audio_soal = audio_soal[0].key;
        } else {
          audio_soal = null;
        }
  ...
  async setSoalBulk(req, res, next){
    ...
    try {      
      ...
      if(!id_matkul) throw createError.BadRequest('id matkul tidak boleh kosong!');
      const excelFile = pathAll(req.files.soal_bulk[0].key, 'xlsx');
      const workbook = new ExcelJS.Workbook();
      const wb = await workbook.xlsx.read(excelFile);
      ...
      for (const image of ws.getImages()) {
        ...
        let file = {
          fileKey: `${row}.${col}.${img.name}.${img.extension}`,
          buffer: img.buffer,
          mimetype: 'image/png'
        }
        let s3 = await uploadFile(file);        
        if(s3 instanceof Error){
          for(let i = 0; i < 5; i++){
            s3 = await uploadFile(file);
            if(i !== 5 && s3 instanceof Error){
              console.error(s3)
              continue;
            } else if(i === 5 && s3 instanceof Error){
              console.error(s3);
              throw createError.Unprocessable(`Gagal mengirim gambar baris ke: ${row} 
              dan kolom ke: ${col}, ke s3`); 
            } else {
              imgArray.push({[row]:`${row}.${col}.${img.name}.${img.extension}`});
              picPatharr.push({Key:`images/${row}.${col}.${img.name}.${img.extension}`});
              break;
            }
          }
        } else {
      ...
      res.status(200).json({
        success: true,
        msg: 'data berhasil diimport ke DB sesuai: ' + req.files.soal_bulk[0].key
      });
    } catch (error) {
      if(req.files.soal_bulk) {
        deleteFiles([{Key:`excels/${req.files.soal_bulk[0].key}`}]);
      }
      if(picPatharr.length){
        deleteFiles(picPatharr);
      }
      if(req.audioPatharr && req.audioPatharr.length){
        deleteFiles(req.audioPatharr);
      }
      if(req.videoPatharr && req.videoPatharr.length){
        deleteFiles(req.videoPatharr);
      }      
      next(error);
    }
  },
  ...
  async putSoal(req, res, next){
    try {
      ...
      if(dosen){
        ...
        let { gambar_soal_1, gambar_soal_2, gambar_soal_3, audio_soal, video_soal } = req.files;
        if(gambar_soal_1){array_gambar_soal[0] = gambar_soal_1[0].key}
        if(gambar_soal_2){array_gambar_soal[1] = gambar_soal_2[0].key}
        if(gambar_soal_3){array_gambar_soal[2] = gambar_soal_3[0].key}
        if(req.body.audio_soal) {
          audio_soal = req.body.audio_soal;
        } else if(audio_soal) {
          audio_soal = audio_soal[0].key;
  ...
  async putSoalBulk(req, res, next){
    ...
    try {      
      ...
      if(req.audioPatharr.length){
        deleteFiles(audioPatharr);
      }
      if(req.videoPatharr.length){
        deleteFiles(videoPatharr);
      }
      next(error);
    }
  },
